...
如果经常对不同版本资源重复同一套勾选，可以用模板复用上次的选择：

- 每次导出成功后，当前勾选与导出模式会自动记录为「上次操作」。
- 在底部导出栏点「模板」，可套用「上次操作」，或把当前勾选另存为命名模板。
- 命名模板最多保存 20 条，同名保存视为覆盖，可随时删除。
- 模板保存的是勾选的主 token 与导出模式；导出目录、权限方式属于全局设置，不随模板变化。

套用时按 token 精确匹配当前 Bundle：

- 能匹配的 token 直接勾选，导出模式一并还原。
- 新版本资源里已不存在的 token 会被跳过，界面提示「匹配 N 项，缺失 M 项」，点「查看缺失」可展开缺失清单。
